...
df_selection = df.query("Quarter == @Quarter")
...
fig1 = go.Figure(go.Indicator(domain = {'x': [0, 1], 'y': [0, 1]}, value = int(df_selection["Total amount GBP"].sum()), mode = "gauge+number+delta", title = {'text': "Sales target; Breakeven: 200K"}, delta = {'reference': 200000, "prefix": "2M;"}, gauge = {'axis': {'range': [None, 400000]},'steps' : [{'range': [0, 100000], 'color': "tomato"},{'range': [100000, 250000], 'color': "orange"}],'threshold' : {'line': {'color': "red", 'width': 4}, 'thickness': 0.75, 'value': 350000}}))
st.plotly_chart(fig1, use_container_width=True)
st.markdown("---")
fig_account_income = px.pie(df_selection, values = "Total amount GBP", names = "Account Group", hole =0.5)
...
fig_account_income1 = px.pie(df_selection, values = "Total amount GBP", names = "Service Types", hole =0.5)
